...
<div class="container py-5">
    ...
    <div class="border px-4 py-4 mb-3 rounded-lg shadow-lg">
        ...
        <form method="POST" action="{{ route('dashboard-user.updateFile', ['tambahtugas' => $tambahtugas->id]) }}" enctype="multipart/form-data">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Maksimal 10 MB (Format: JPG, JPEG, PNG, PDF)</p>
            <button type="submit" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 mt-3 flex items-end mx-auto justify-end">Simpan</button>
        </form>
        @endif
    </div>    
</div>
